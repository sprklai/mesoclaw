...
const CONFIG_KEY_OPTIONS = [
  {
    value: "context_injection_enabled",
    label: "wb_option_context_injection_enabled",
  },
  {
    value: "self_evolution_enabled",
    label: "wb_option_self_evolution_enabled",
  },
...
export const nodeRegistry = {
  ...
  getVisible(capabilities: string[]): NodeDefinition[] {
    ...
    return NODE_DEFINITIONS.filter(
      (def) => !def.hidden && (!def.featureGate || caps.has(def.featureGate)),
    );
  },
};
