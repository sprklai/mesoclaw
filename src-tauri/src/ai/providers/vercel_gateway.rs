...
/// Default base URL for Vercel AI Gateway
...
/// Configuration for Vercel AI Gateway Provider
#[derive(Debug, Clone)]
...
impl Default for VercelAIGatewayConfig {
    fn default() -> Self {
        Self {
            api_key: String::new(),
...
///
/// Provides access to multiple LLM providers through Vercel's unified API:
/// - OpenAI (gpt-4, gpt-3.5-turbo, etc.)
/// - Anthropic (claude-sonnet-4.5, claude-haiku-4.5, etc.)
/// - Google (gemini-pro, gemini-flash, etc.)
/// - xAI (grok-code-fast-1, etc.)
/// - And many more
///
...
impl VercelAIGatewayProvider {
    /// Create a new Vercel AI Gateway provider
    pub fn new(config: VercelAIGatewayConfig) -> Result<Self> {
...
#[async_trait]
impl LLMProvider for VercelAIGatewayProvider {
    ...
    fn supports_tools(&self) -> bool {
        // Vercel AI Gateway supports function calling for compatible models
        true
    }
...
mod tests {
    ...
    fn test_config_builder() {
        let config = VercelAIGatewayConfig::new("test-key")
        ...
        assert_eq!(config.default_model, "openai/gpt-4");
    ...
    fn test_message_conversion() {
        ...
        let api_msg = VercelAIGatewayProvider::convert_message(&system_msg);
        assert_eq!(api_msg.role, "system");
        assert_eq!(api_msg.content, "You are helpful");

        let user_msg = AppMessage::user("Hello");
        let api_msg = VercelAIGatewayProvider::convert_message(&user_msg);
        assert_eq!(api_msg.role, "user");
        assert_eq!(api_msg.content, "Hello");

        let assistant_msg = AppMessage::assistant("Hi there");
        let api_msg = VercelAIGatewayProvider::convert_message(&assistant_msg);
    ...
    fn test_context_limits() {
        // Anthropic models
        assert_eq!(
            VercelAIGatewayProvider::get_context_limit("anthropic/claude-sonnet-4.5"),
            200_000
        );
        assert_eq!(
            VercelAIGatewayProvider::get_context_limit("anthropic/claude-haiku-4.5"),
            200_000
        );

        // OpenAI models
        assert_eq!(
            VercelAIGatewayProvider::get_context_limit("openai/gpt-4o"),
            128_000
        );
        assert_eq!(
            VercelAIGatewayProvider::get_context_limit("openai/gpt-4"),
            8_192
        );
        assert_eq!(
            VercelAIGatewayProvider::get_context_limit("openai/gpt-3.5-turbo"),
            4_096
        );

        // Google models
        assert_eq!(
            VercelAIGatewayProvider::get_context_limit("google/gemini-2-flash"),
            1_000_000
        );

        // xAI models
        assert_eq!(
            VercelAIGatewayProvider::get_context_limit("xai/grok-code-fast-1"),
            128_000
        );

        // Unknown models
        assert_eq!(
            VercelAIGatewayProvider::get_context_limit("unknown/model"),
            4_096
        );
    }
    ...
    fn test_provider_creation() {
        let config = VercelAIGatewayConfig::new("test-key");
        let provider = VercelAIGatewayProvider::new(config);
    ...
    fn test_provider_traits() {
        let config = VercelAIGatewayConfig::new("test-key");
        let provider = VercelAIGatewayProvider::new(config).unwrap();

        assert_eq!(provider.provider_name(), "vercel-ai-gateway");
        assert!(provider.supports_tools());
        assert_eq!(provider.context_limit(), 1_000_000); // Default model is gemini-3-flash
    }
    ...
    fn test_build_request() {
        ...
        assert_eq!(api_request.model, "anthropic/claude-sonnet-4.5");
