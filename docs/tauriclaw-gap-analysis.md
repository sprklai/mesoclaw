# TauriClaw Gap Analysis: Current State vs. Recommended Architecture

> Compares existing TauriClaw codebase against patterns recommended in `docs/claw-ecosystem-analysis.md`.
> Reference: ZeroClaw (Rust), PicoClaw (Go), IronClaw (Rust), OpenClaw (TypeScript).
> **v2 Update (Feb 2026)**: Added missing OpenClaw core features — Event Bus, Heartbeat/Scheduler, Identity Files, Proactive Notifications, Session Management, Daily Memory. Architecture follows "Gateway-Ready" approach (Approach B): trait-based abstractions that ship with Tauri IPC + Tokio tasks today, but support adding WebSocket/webhook/external channels later without refactoring.
...
| Tauri IPC commands layer | 18+ commands registered, clean `Result<T, String>` pattern | `src-tauri/src/commands/` |
...
TauriClaw follows a **Gateway-Ready** architecture: trait-based abstractions for all subsystems, shipping with Tauri IPC + Tokio background tasks as the default implementations, but designed so WebSocket gateways, webhook listeners, and external channel adapters can be added later without refactoring core logic.
...
│  (EventBus trait → tokio::broadcast + Tauri emit)    │
├────────────┬─────────────┬───────────────────────────┤
│  Agent     │  Scheduler  │  Channel Manager          │
│  Loop      │  (heartbeat │  (tauri ipc, webhook,     │
│  (P0.3)    │   + cron)   │   future: ws, telegram)   │
├────────────┴─────────────┴───────────────────────────┤
│            Core Services                              │
...
**Key principle**: Every subsystem is a trait. Default implementations use Tauri-native mechanisms. Future implementations can use WebSocket, HTTP, or platform SDKs — plugged in via config without touching consumer code.

**Desktop vs Server distinction**: Unlike OpenClaw (server-based with WebSocket control plane), TauriClaw is a desktop app. The "gateway" is Tauri's IPC bridge. The Event Bus provides internal routing between subsystems. External channels (webhook, WebSocket) are optional add-ons, not the default path.
...
**Replace with**:
- Prompt templates as `.md` files in `~/.tauriclaw/prompts/` (or embedded via `include_str!()`)
...
**Problem**: Diesel is a production-grade ORM designed for multi-database, multi-user server applications. TauriClaw is a single-user desktop app using SQLite exclusively. The ORM adds:
...
- Blocked system directories: `/etc`, `/root`, `/proc`, `/sys`, `/dev`, `/var`, `/tmp`, `~/.ssh`, `~/.gnupg`, `~/.aws`
...
**Gap**: No channel abstraction. The app communicates only via Tauri IPC (WebView ↔ Rust). There's no way to receive messages from webhooks, WebSocket clients, or external messaging platforms.
...
- `src-tauri/Cargo.toml` - Add `axum` (0.8) + `tower-http` only if HTTP gateway needed
...
**Design note**: The EventBus is the backbone that connects all other new subsystems. The heartbeat publishes `HeartbeatTick` events, the agent loop publishes tool execution events, channels publish inbound messages, and the Tauri bridge forwards UI-relevant events to the frontend. This replaces the need for a WebSocket control plane in a desktop context.
...
**What's needed**:
- `Identity` struct loaded from markdown files in `~/.tauriclaw/identity/`:
...
**What's needed**:
- TOML config file at `~/.tauriclaw/config.toml`
- Serde deserialization with `#[serde(default)]` for all fields
- Environment variable override support (e.g., `TAURICLAW_PROVIDER=anthropic`)
...
**What's needed**:
- Daily memory directory: `~/.tauriclaw/memory/`
...
# P1.7 - Channel system (only if HTTP gateway needed)
axum = "0.8"
tower-http = "0.6"
...
# P1.5 - Memory system
# No new deps needed - uses existing rusqlite (via diesel), lru, reqwest
...
**Moltis as pattern reference.** While neither Moltis nor MicroClaw can be imported as dependencies (both are applications, not libraries), Moltis's memory system, agent loop, hook architecture, and strict linting provide excellent reference implementations for TauriClaw's new modules.
...
| OpenClaw Core Feature | TauriClaw Implementation | Status |
|---|---|---|
