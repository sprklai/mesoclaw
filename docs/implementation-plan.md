# TauriClaw Implementation Plan & Task List

> Step-by-step execution plan for transforming TauriClaw from a chat application into a full desktop AI agent.
...
+ Responsive    (2 items)      (10 items)        (3 items)         (4 items)        (5 items)       & UI (6)       Mobile (4)     Community (5)
(8 items)       2 files        ~37 new files     ~10 new files     ~11 new files    ~7 new files    ~16 files      ~17 files      ~19 files
```

**Total**: 46 items across 8 phases. ~135 new files, ~15 deleted.
...
| 0.3.5 | Create `src-tauri/src/prompts/loader.rs` | Load templates from: (1) embedded via `include_str!()`, (2) filesystem at `~/.tauriclaw/prompts/`. ~100-150 lines total |
...
| # | Task | Details |
|---|------|---------|
| 0.4.1 | Add `rusqlite` to `src-tauri/Cargo.toml` | `rusqlite = { version = "0.32", features = ["bundled"] }` |
...
| 0.6.6 | Verify both binaries compile | `cargo build --bin tauriclaw` (CLI) and `cargo build --bin tauriclaw-desktop` (GUI) |
...
| 0.7.5 | Verify: `cargo run --bin tauriclaw -- --help` shows all commands | |
...
- [ ] Two binaries compile: `cargo build --bin tauriclaw` and `cargo build --bin tauriclaw-desktop`
- [ ] `tauriclaw --help` shows full command structure
...
| 2.4.4 | Implement `IdentityLoader` in `loader.rs` | Load from `~/.tauriclaw/identity/`. Fall back to embedded defaults if file doesn't exist. Create directory and copy defaults on first run |
...
| 2.5.3 | Implement bearer token auth middleware in `auth.rs` | Generate random token on startup, write to `~/.tauriclaw/daemon.token` (mode 0600). Validate `Authorization: Bearer` header |
...
| 2.8.5 | Implement `ModuleRegistry` in `mod.rs` | Scan `~/.tauriclaw/modules/` for manifests. Register discovered modules in ToolRegistry. Lifecycle management (start services on boot, lazy tools) |
...
| 2.8.8 | Write unit tests (minimum 15) | Test: manifest parsing, module discovery, SidecarTool spawn/execute/kill, protocol framing, registry integration, health checks |
...
- [ ] CLI: `tauriclaw agent status` returns session list
...
| # | Task | Details |
|---|------|---------|
| 3.3.1 | Create `src-tauri/src/memory/daily.rs` | `DailyMemory` struct. Manages `~/.tauriclaw/memory/` directory |
...
| 3.3.7 | Create `MEMORY.md` support | Curated long-term memory file at `~/.tauriclaw/memory/MEMORY.md`. Loaded into system prompt. User can edit manually or agent can update via memory tool |
...
| 5.1.3 | Implement config loading in `src-tauri/src/config/mod.rs` | Load from `~/.tauriclaw/config.toml`. Override with env vars (`TAURICLAW_*`). Fall back to defaults if file doesn't exist |
...
| # | Task | Details |
|---|------|---------|
| 5.5.1 | Add `module` subcommand group to CLI | `tauriclaw module list/install/remove/start/stop/health/reload/create` |
| 5.5.2 | Implement `module list` | Show registered modules, type (tool/service/mcp), runtime, status (running/stopped/error) |
| 5.5.3 | Implement `module create` scaffolding | `tauriclaw module create <name> --type tool --runtime python` — generates manifest.toml + template script |
...
| 6.2.2 | Implement startup sequence | Order: (1) Create `~/.tauriclaw/` directory structure. (2) Load config. (3) Load identity files. (4) Load daily memory. (5) Warm up providers. (6) Start scheduler. (7) Start channels. (8) Execute BOOT.md checklist (if exists). (9) Emit `SystemReady` event |
...
| 7.4.2 | Configure iOS build | Set bundle identifier (`com.nsrtech.tauriclaw`), app icons (all required sizes), Info.plist (camera, microphone, file access permissions). Configure code signing for development (Apple Developer account). Test on iOS Simulator (iPhone 15, iPad) |
| 7.4.3 | Configure Android build | Set package name (`com.nsrtech.tauriclaw`), app icons, AndroidManifest.xml (internet, file access, notification permissions). Configure signing for debug build. Test on Android emulator (Pixel 7, Galaxy Tab) |
