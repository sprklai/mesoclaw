# Mesoclaw Product Requirements Document (PRD)

> Defines what Mesoclaw is, who it's for, what it does, and how success is measured.
...
**Mesoclaw** is a lightweight, privacy-first desktop AI agent that runs natively on macOS, Windows, and Linux. It combines the power of cloud LLMs (or local models via Ollama) with the security and responsiveness of a native application. Unlike browser-based AI tools, Mesoclaw runs on your machine, stores data locally, and can act autonomously — reading files, executing commands, monitoring services, and learning from interactions over time.
...
| FR-4.8  | Daily memory files: `~/.mesoclaw/memory/YYYY-MM-DD.md`                           | P2       | Planned (P2.17) |
...
| ID     | Requirement                                                                                   | Priority | Status          |
| ------ | --------------------------------------------------------------------------------------------- | -------- | --------------- |
| FR-5.1 | Identity files in `~/.mesoclaw/identity/`: SOUL.md, USER.md, AGENTS.md, IDENTITY.md, TOOLS.md | P1       | Planned (P1.10) |
...
| ID      | Requirement                                                          | Priority | Status          |
| ------- | -------------------------------------------------------------------- | -------- | --------------- |
| FR-11.1 | TOML config file at `~/.mesoclaw/config.toml`                        | P2       | Planned (P2.11) |
| FR-11.2 | Environment variable overrides (e.g., `MESOCLAW_PROVIDER=anthropic`) | P2       | Planned (P2.11) |
...
| FR-14.2  | One-shot mode: `mesoclaw "prompt"` runs agent, prints result, exits          | P0       | Planned |
| FR-14.3  | Stdin pipe support: `cat file \| mesoclaw "analyze"`                         | P0       | Planned |
...
| FR-14.7  | Daemon management: `mesoclaw daemon start/stop/status/logs`                  | P0       | Planned |
| FR-14.8  | Agent management: `mesoclaw agent status/stop/logs/list`                     | P0       | Planned |
| FR-14.9  | Memory CLI: `mesoclaw memory search/store/daily`                             | P1       | Planned |
| FR-14.10 | Config CLI: `mesoclaw config show/set`                                       | P1       | Planned |
| FR-14.11 | Watch mode: `mesoclaw watch ./src --prompt "review changes"`                 | P2       | Planned |
...
| FR-14.14 | Session resume: `mesoclaw --resume session-id`                               | P1       | Planned |
...
| FR-15.4 | Auto-generated bearer token in ~/.mesoclaw/daemon.token               | P0       | Planned |
| FR-15.5 | Port auto-increment (18790-18799) on conflict                         | P1       | Planned |
| FR-15.6 | PID file at ~/.mesoclaw/daemon.pid with port info                     | P0       | Planned |
...
| FR-18.3  | TOML manifest format for module definition (`~/.mesoclaw/modules/{name}/manifest.toml`)            | P1       | Planned |
...
| FR-18.9  | Module discovery: scan `~/.mesoclaw/modules/` on startup, register in ToolRegistry                 | P1       | Planned |
| FR-18.10 | Module security: `allowed_paths`, `network` flag, `timeout_seconds`, `max_memory_mb` per manifest  | P1       | Planned |
| FR-18.11 | Module CLI: `mesoclaw module list/install/remove/start/stop/health/reload/create`                  | P2       | Planned |
...
| FR-18.15 | Module scaffolding: `mesoclaw module create <name> --type tool --runtime python`                   | P2       | Planned |
...
| WhatsApp/Discord/Slack channels                           | Messaging channels begin after v1.0 (Telegram first in v1.1, others after)                                               |
...
| **v0.7** (Core Agent)            | P1.4, P1.6, P1.8, P1.10: Tools, security, event bus, identity. **Module system core** (FR-18.1-18.10, FR-18.14)                           | Phase 2   |
...
| **v0.9** (Proactive)             | P1.9, P2.15, P2.16: Scheduler, notifications, sessions                                                                                    | Phase 4   |
| **v1.0** (Complete)              | P2.11-P2.14, P3.\*: Config, routing, UI polish, boot sequence, channel management UI. **Module management UI** (FR-18.11-18.12, FR-18.15) | Phase 5-6 |
| **v1.1** (Channels+)             | **Telegram channel** (P7.1), channel management UX, and mobile-specific polish (safe areas, gestures)                                     | Phase 7   |
| **v1.2** (Mobile & Distribution) | Tauri Mobile builds (iOS/Android/Tablets), push notifications, TestFlight + Play Console, **CI/CD + signing hardening** (FR-16.1-16.7)    | Phase 7-8 |
