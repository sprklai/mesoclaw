...
**User Goal**: Install and start using Mesoclaw from the terminal immediately.

| Step | User Action                                  | System Response                                                                     | Key Module             |
| ---- | -------------------------------------------- | ----------------------------------------------------------------------------------- | ---------------------- |
| 0.1  | `cargo install mesoclaw` or downloads binary | Single binary installed to PATH                                                     | Release binary         |
| 0.2  | `mesoclaw`                                   | Daemon starts, interactive REPL opens. Prompts for provider + API key on first run. | CLI (clap + rustyline) |
| 0.3  | `mesoclaw "explain this codebase"`           | One-shot: agent analyzes, streams result, exits                                     | Gateway + Agent Loop   |
| 0.4  | `cat schema.sql \| mesoclaw "review this"`   | Piped input as context, agent responds                                              | Pipe support           |
| 0.5  | `mesoclaw gui`                               | Launches Tauri desktop app (connects to running daemon)                             | Tauri GUI              |
...
- **Non-developers**: Run `mesoclaw gui` once. Use GUI exclusively. Same agent, same memory.

---

## Stage 1: First Launch & Onboarding

**User Goal**: Install Mesoclaw and get it running with minimal friction.

| Step | User Action                            | System Response                                                                                                    | Key Module                  |
| ---- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------ | --------------------------- |
| 1.1  | Downloads installer (DMG/MSI/AppImage) | Platform-native installer runs                                                                                     | Tauri bundler               |
| 1.2  | Launches app for first time            | Boot sequence executes: loads defaults, creates `~/.mesoclaw/` directory structure                                 | Boot Sequence (P3.23)       |
...
| 2.2  | Presses Enter / Send          | Message sent to local gateway (`POST /api/v1/...`)                                                            | Gateway + Agent API             |
| 2.3  | Sees streaming response       | Tokens and tool status stream over WebSocket (`/api/v1/ws`), rendered incrementally in Conversation component | Gateway WebSocket               |
...
**User Goal**: Make the agent feel like _their_ assistant — personalized, context-aware, persistent.
...
| 4.4  | Notices agent remembers yesterday's conversation                           | Daily memory files (`~/.mesoclaw/memory/2026-02-16.md`) persist key facts             | Daily Memory (P2.17)       |
...
**Desktop Advantage**: Unlike server-based assistants (OpenClaw), Mesoclaw uses native OS notifications, system tray, and runs without a browser. The agent feels like a native desktop companion.

---

## Stage 6: Daily Power Usage

**User Goal**: Mesoclaw is an indispensable part of the daily workflow.
...
| 7a.2 | Creates bot via BotFather, copies token | Pastes bot token into Mesoclaw. Token stored in OS keyring                                                                 | Credential Store               |
...
| Step | User Action                                                       | System Response                                                                                                      | Key Module                         |
| ---- | ----------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- | ---------------------------------- |
| 8.1  | Installs Mesoclaw from TestFlight (iOS) or Play Console (Android) | Same React frontend compiled via Tauri Mobile. No code changes from desktop — responsive layout adapts automatically | Tauri Mobile + Responsive Layout   |
...
## Stage 9: Contributing to Mesoclaw
...
| Step  | User Action                                                          | System Response                                                                                                | Key Module               |
| ----- | -------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | ------------------------ |
| 10a.1 | `mesoclaw module create python-analyst --type tool --runtime python` | Scaffolds `~/.mesoclaw/modules/python-analyst/` with `manifest.toml` template and `main.py` stub               | Module CLI               |
| 10a.2 | Edits `main.py` with custom analysis logic                           | Python script reads stdin JSON, processes, writes stdout JSON                                                  | User code                |
| 10a.3 | `mesoclaw module reload`                                             | Module system re-scans modules directory, discovers new module, registers tool in ToolRegistry                 | ModuleRegistry           |
| 10a.4 | `mesoclaw "analyze sales.csv using python-analyst"`                  | Agent calls `python-analyst` tool → spawns Python process → sends request → receives result → presents to user | SidecarTool + Agent Loop |
...
| 10b.2 | `mesoclaw module reload`                                                                    | Container runtime auto-detected (Podman or Docker). Image pulled if needed                                                                          | ContainerRuntime               |
...
| 10c.2 | Creates `~/.mesoclaw/modules/composio/manifest.toml` with `type = "mcp"` and `command = "npx composio-core mcp-server"` | Manifest defines MCP server module                                                                                       | Manifest                |
| 10c.3 | `mesoclaw module reload`                                                                                                | MCP client starts Composio server → sends `tools/list` → discovers 500+ tools → registers selected tools in ToolRegistry | McpServer + MCP Client  |
| 10c.4 | `mesoclaw "send an email via Gmail about today's summary"`                                                              | Agent calls `mcp:composio:gmail_send` → MCP client sends `tools/call` → Composio handles OAuth + API call → email sent   | MCP Client + Agent Loop |
