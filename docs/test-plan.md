# TauriClaw Comprehensive Test Plan
...
| U-0.1.3 | `test_feature_flag_modules_enabled` | `modules` feature flag enabled → module code compiles | Feature | `modules/mod.rs` |
| U-0.1.4 | `test_feature_flag_modules_disabled` | `modules` feature disabled → module code excluded | Feature | CI only |
...
| U-0.1.6 | `test_feature_flag_default_set` | Default features include modules + containers | Feature | `Cargo.toml` |
...
| U-0.3.2 | `test_template_loads_from_filesystem` | Templates in `~/.tauriclaw/prompts/` are discovered and loaded | `prompts/loader.rs` |
...
| Test ID | Test Name | Description | Pass Criteria |
|---------|-----------|-------------|---------------|
| T-0.6.1 | CLI binary compiles | `cargo build --bin tauriclaw` | Exits 0, binary exists |
| T-0.6.2 | Desktop binary compiles | `cargo build --bin tauriclaw-desktop` | Exits 0, binary exists |
...
| Test ID | Test Name | Description | Location |
|---------|-----------|-------------|----------|
| U-0.7.1 | `test_cli_parses_agent_status` | `tauriclaw agent status` parsed as AgentCommand::Status | `bin/cli.rs` |
| U-0.7.2 | `test_cli_parses_one_shot` | `tauriclaw "analyze this"` parsed as default run command | `bin/cli.rs` |
...
| Test ID | Test Name | Description | Pass Criteria |
|---------|-----------|-------------|---------------|
| T-0.7.1 | Help output | `cargo run --bin tauriclaw -- --help` | Lists all subcommands, exits 0 |
| T-0.7.2 | Version output | `cargo run --bin tauriclaw -- --version` | Shows version string, exits 0 |
| T-0.7.3 | Unknown subcommand | `cargo run --bin tauriclaw -- invalid` | Error message, exits non-zero |
...
|---------------|-----------|-------|-----------------|
| M-0.1 | **Fresh install experience** | 1. Delete `~/.tauriclaw/`. 2. Run `tauriclaw`. 3. Observe first-run behavior. | CLI prompts for provider + API key. Creates config directory. |
...
| M-0.5 | **CLI help is comprehensive** | 1. Run `tauriclaw --help`. 2. Run `tauriclaw agent --help`. 3. Run `tauriclaw memory --help`. | Each shows clear subcommands and descriptions |
...
| Wave Test ID | Test Name | Description | Pass Criteria |
|-------------|-----------|-------------|---------------|
| W-1.1 | **Release binary size** | `cargo build --release` → `ls -la target/release/tauriclaw` | < 15 MB |
...
| U-2.5.13 | `test_pid_file_written` | Start daemon → ~/.tauriclaw/daemon.pid exists with correct JSON | `gateway/daemon.rs` |
...
| Wave Test ID | Test Name | Description | Pass Criteria |
|-------------|-----------|-------------|---------------|
| W-2.1 | **Gateway starts and responds** | Start daemon → `curl -H "Authorization: Bearer $(cat ~/.tauriclaw/daemon.token)" http://127.0.0.1:18790/api/v1/health` | Returns `{"status": "ok"}` |
| W-2.2 | **CLI connects to gateway** | Start daemon → `tauriclaw agent status` | Returns session list (may be empty) |
| W-2.3 | **CLI REPL streams response** | Start daemon → `tauriclaw` → type message → observe streaming | Tokens stream character by character |
...
| W-2.9 | **Module system discovers modules** | Create module in `~/.tauriclaw/modules/` → restart → verify tool registered | Tool appears in `tauriclaw module list` |
...
| M-2.2 | 1. `tauriclaw`. 2. Type "list files in current directory". 3. Observe tool execution. 4. Type "y" to approve. | Agent calls `ls`, shows results. Approval prompt appears (if Supervised). |
| M-2.3 | 1. Edit `~/.tauriclaw/identity/SOUL.md` to say "Always respond in haiku format". 2. Start new CLI session. 3. Ask a question. | Agent responds in haiku format. |
| M-2.4 | 1. Start daemon. 2. Open GUI via `tauriclaw gui`. 3. Send message in GUI. 4. Send message in CLI simultaneously. | Both work. Both use same gateway. Separate sessions. |
| M-2.5 | 1. Ask agent to read `/etc/passwd`. 2. Ask agent to read `~/.ssh/id_rsa`. | Both denied by SecurityPolicy. Clear error message. |
| M-2.6 | 1. `tauriclaw module create test-tool --type tool --runtime python`. 2. Edit main.py to echo input. 3. `tauriclaw module reload`. 4. `tauriclaw "use test-tool to process data"`. | Agent calls test-tool, Python script executes, result shown |
| M-2.7 | 1. `tauriclaw module list`. 2. Verify all registered modules shown with status. | Table shows name, type, runtime, status for each module |
...
| U-3.2.3 | `test_hybrid_search_scoring` | Verify final_score = 0.7 * vector + 0.3 * bm25 | `memory/sqlite.rs` |
...
| W-3.2 | **Agent stores to memory** | Ask agent to "remember that this project uses Tauri 2" → verify in memory search | `tauriclaw memory search "Tauri"` returns the entry |
...
| W-3.4 | **Daily memory persists** | End session → check `~/.tauriclaw/memory/` for today's file | File exists with session highlights |
...
| Manual Test ID | Steps | Expected Result |
|---------------|-------|-----------------|
| M-3.1 | 1. `tauriclaw`. 2. "Read and explain the Cargo.toml in this directory". 3. Approve file read. 4. Observe multi-turn. | Agent reads file → explains dependencies → done in 2-3 turns |
| M-3.2 | 1. `tauriclaw`. 2. "Remember that I prefer functional programming style". 3. `/quit`. 4. `tauriclaw`. 5. "Help me refactor this function". | Agent references the functional programming preference |
| M-3.3 | 1. `tauriclaw --auto "find all TODO comments in src/"`. 2. Observe. | Agent runs grep/find autonomously (no approval), returns list |
| M-3.4 | 1. `echo "fn main() { println!(\"hello\"); }" \| tauriclaw "review this code"` | Agent receives piped code, provides review |
| M-3.5 | 1. Check `~/.tauriclaw/memory/$(date +%Y-%m-%d).md` after a session. | File contains timestamped entries from the session |
...
| Test ID | Test Name | Description | Location |
|---------|-----------|-------------|----------|
| U-4.1.1 | `test_cron_parse_every_30_min` | "*/30 * * * *" parses correctly | `scheduler/cron_parser.rs` |
...
| W-4.2 | **Cron job fires** | `tauriclaw schedule add --cron "* * * * *" --prompt "say hello"` → wait 60s | Job executes, result visible in history |
...
| Manual Test ID | Steps | Expected Result |
|---------------|-------|-----------------|
| M-4.1 | 1. `tauriclaw schedule add --cron "*/2 * * * *" --prompt "check system health"`. 2. Wait 4 min. 3. `tauriclaw schedule list`. | Shows 2 executions in history |
| M-4.2 | 1. Edit `~/.tauriclaw/identity/HEARTBEAT.md`: add "Check if any test files were modified today". 2. Wait for heartbeat. | Heartbeat checks for modified test files |
...
| Wave Test ID | Test Name | Description | Pass Criteria |
|-------------|-----------|-------------|---------------|
| W-5.1 | **TOML config round-trip** | `tauriclaw config set provider.default anthropic` → `tauriclaw config show` → shows anthropic | Value persisted and retrieved |
| W-5.2 | **Env override** | `TAURICLAW_PROVIDER=ollama tauriclaw config show` → shows ollama as active | Env var takes priority over file |
...
| Manual Test ID | Steps | Expected Result |
|---------------|-------|-----------------|
| M-6.1 | 1. Start fresh (delete ~/.tauriclaw). 2. Launch GUI. 3. Observe boot. | Setup wizard appears. Default identity created. Provider prompt shown. |
...
| W-7.2 | **Telegram + Desktop memory shared** | Chat in Telegram: "remember X" → Desktop CLI: `tauriclaw memory search "X"` | Memory entry found from CLI |
...
| Manual Test ID | Steps | Expected Result |
|---------------|-------|-----------------|
| M-7.1 | 1. Create Telegram bot via @BotFather. 2. `tauriclaw channel connect telegram`. 3. Enter bot token. 4. Send message to bot. | Bot responds with agent's answer |
...
|------------|-----------|-------------|---------------|
| E2E-1 | **Developer quick start** | Fresh install → `tauriclaw` → configure provider → "analyze this project" → agent reads files → responds | Working in < 3 minutes. Agent response references actual project files. |
| E2E-2 | **GUI non-developer onboarding** | Fresh install → `tauriclaw gui` → setup wizard → first chat → response | Working in < 3 minutes. No terminal knowledge needed. |
| E2E-3 | **CLI pipe workflow** | `git diff \| tauriclaw "review this diff" --raw > review.md` | review.md contains meaningful code review |
...
| E2E-6 | **Session resume** | `tauriclaw` → multi-turn conversation → `/quit` → `tauriclaw --resume <id>` → continue | Full conversation context restored |
| E2E-7 | **Autonomous task** | `tauriclaw "find and fix all clippy warnings in src/" --auto` | Agent runs clippy → edits files → runs clippy again → reports clean |
...
jobs:
  ...
  integration-tests:
    - cargo build --bin tauriclaw
    - cargo test --test gateway_integration
    - cargo test --test cli_integration

  build-check:
    - cargo build --release --bin tauriclaw
    - cargo build --release --bin tauriclaw-desktop
...
*References: docs/implementation-plan.md, docs/product-requirements.md*
